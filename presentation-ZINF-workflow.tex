...
\begin{frame}{Continuous Integration with result visualization} 
    ...
    \vfill
    \begin{enumerate}
        \item \textbf{Short small few CPU-core tests}: work-PC \faGraduationCap.    
        \item \textbf{Short small many-core tests}: obtain a workstation with a 64-Core CPU\footnote{Many thanks to \href{https://www.sfb1194.tu-darmstadt.de/index.en.jsp}{CRC 1194 at TU Darmstadt}}\faGraduationCap.
        \item \textbf{Long large HPC tests}: combine 1. or 2. with an HPC cluster. 
    \end{enumerate}
    ...
    \begin{itemize}
        \item This workflow uses coarse ("smoke") tests \faGraduationCap
            ...
            \end{itemize}
        \item \textbf{Challenge}: Is it possible to combine 1., 2. and 3. and keep the workflow simple?
    \end{itemize}
...
\begin{frame}{Continuous Integration with result visualization} 
    ...
    \begin{itemize}
        \item Anything added to the Docker container within a job is gone when the job ends. 
        ...
        \item Applications and libraries are transfered from the build job as artifacts.
    \end{itemize}

\end{frame}

\begin{frame}[fragile]{Continuous Integration with result visualization} 
    \framesubtitle{Building OpenFOAM projects or projects with out-of-repository installation}

    \vfill
    \textbf{Out-of-source installation}: binaries placed outside the repo! 
    \begin{itemize}
        ...
        \item \textbf{Build job}: 
            \begin{itemize}
                \item create folders inside the repo, 
                \item copy installed binaries to artifact folders, 
                \item export artifact folders. 
            \end{itemize}
        \item \textbf{Run job}: 
            \begin{itemize} 
                \item \texttt{mkdir -p} \texttt{\$FOAM\_USER\_LIBBIN} and  \texttt{\$FOAM\_USER\_LIBBIN} 
                \item copy binaries from the artifact folders to \texttt{\$FOAM\_USER\_LIBBIN} and  \texttt{\$FOAM\_USER\_LIBBIN} 
                \item Run tests.
            \end{itemize}
    \end{itemize}
